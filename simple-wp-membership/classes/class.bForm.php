...
class BForm{
	...
	protected function user_name(){
		...
		$query = "SELECT count(member_id) FROM {$wpdb->prefix}wp_eMember_members_tbl WHERE user_name= '" .
				 strip_tags($saned). "'";
	...
	protected function email(){
		...
		$query = "SELECT count(member_id) FROM {$wpdb->prefix}wp_eMember_members_tbl WHERE email= '" .
				 strip_tags($saned). "'";
		$result = $wpdb->get_var($query);
		if($result>0){
