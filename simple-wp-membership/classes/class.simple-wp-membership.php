...
class SimpleWpMembership{
	...
	public static function swpm_login($user, $pass, $rememberme = true){
		if(is_user_logged_in()){
			...
			if($current_user->user_login == $user) return;
		}
	...
	public function wp_login($username,$password){
		...
		if($auth->is_logged_in()){
			if($auth->userData->user_name == $username) return;
		}
		$auth->login($username,$password, true);
	}
	public function wp_logout(){
		$auth = BAuth::get_instance();
		if($auth->is_logged_in())$auth->logout();		
	}
	...
	public function notices(){
		...
		if(!empty($message)){
			...
			if(!empty($extra)){
				echo '<ul>'; 
				foreach($extra as $key=>$value)echo '<li>' . $value . '</li>';
				echo '</ul>';
			}
			echo  "</div>";
		}
	...
	public function save_postdata($post_id){
		...
		if(!isset($_POST['swpm_protect_post'])) return; 
		...
		if(isset($_POST['post_type'])){
            ...
            foreach($level_ids as $level)
                ...
                isset($_POST['swpm_protection_level'][$level]), $_POST['post_type'])->save();
		}
        ...
        if(isset($_POST['swpm-reset'])){
            if(is_email($_POST['swpm_reset_email']))
                ...
                if(empty($user))
                    ...
                    $wpdb->update($wpdb->prefix . "wp_eMember_members_tbl", 
                                  array('password'=>$password),
                                  array('member_id'=>$user->member_id));
