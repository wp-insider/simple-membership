...
- Fixed a Stripe SCA payment button issue where an unencoded reference ID in the checkout success URL could cause a "Payment button (ID: 0) does not exist" error after a completed payment.
...
- Password reset via a secure link is now enabled by default on new installations. Site administrators can disable it using the new Disable Password Reset Using Link option under Settings.
