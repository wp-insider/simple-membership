...
declare const swpmPasswordToggleStrings: Record<string, string>;

type CheckboxTogglerStyles = {
...
type Options = {
    formId: string,
    type: string,
    checkboxTogglerSelector: string,
    checkboxTogglerStyles: CheckboxTogglerStyles,
    passwordInputSelectors: string,
    passwordInputSelectorsToAttach: string,
}

type IconClassNames = string[];
...
class SWPM_Password_Visibility_Toggler {
    ...
    passwordInputsToAttach: NodeListOf<HTMLInputElement>;

    constructor(options: Options) {
        options = this.parseOptions(options);

        this.type = options.type.trim().toLowerCase();
        this.form = document.getElementById(options.formId.trim()) as HTMLFormElement;
    ...
    parseOptions = (options: Partial<Options>) => {
        ...
        const mergedOptions: Options = { ...defaults };
    ...
    setOption = <K extends keyof Options>( target: Options, source: Partial<Options>, defaults: Options, key: K)=> {
    ...
    attachCheckboxToggler = (input: HTMLInputElement) => {
        ...
        if(this.checkboxTogglerSelector){
            ...
            checkbox?.addEventListener('change', e => this.onCheckboxChange(e, input));
            return;
        }
    ...
    attachIconToggler = (input: HTMLInputElement) => {
    ...
    onCheckboxChange = (e: Event, input: HTMLInputElement) => {
        e.preventDefault();
    ...
    onIconClick = (e: Event) => {
        let icon = e.target as HTMLElement|null;
        if (!icon?.matches('.swpm-form-input-icon')) { // Check if user clocked on svg/path, not the wrapper span
            icon = icon?.closest('.swpm-form-input-icon') || null;
        }
        ...
        const input = icon.parentNode?.querySelector( 'input' ) as HTMLInputElement;
    ...
    positionPasswordIcons = () => {
        ...
        const passToggleIcons = document.querySelectorAll(iconSelector) as NodeListOf<HTMLElement>;

        passToggleIcons.forEach(this.positionPasswordIcon);
    }

    positionPasswordIcon = (icon: HTMLElement) =>{
        if (!icon) {
            return;
        }

        const wrap = icon.parentNode as HTMLElement;

        const input = wrap.querySelector<HTMLInputElement>(this.passwordInputSelectorsToAttach);
        if (!input) {
            return;
        }
        ...
        const inputOffsetRight = (input.offsetParent as HTMLElement).offsetWidth - (input.offsetLeft + input.offsetWidth);
     ...
     * @returns {string}
     */
    getClassSelectorFromArray(classnames: IconClassNames){
